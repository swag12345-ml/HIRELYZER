...
def evaluate_interview_answer(answer: str):
    ...
    Improvement 1: <specific suggestion for improvement>
    Improvement 2: <another specific suggestion for improvement>
    ...
    try:
        ...
        # Extract Feedback
        feedback_match = re.search(r"Feedback:\s*(.+?)(?=Improvement|$)", response, re.DOTALL)
        feedback = feedback_match.group(1).strip() if feedback_match else "Good attempt, but can be clearer."

        # Extract Improvements
        improvements = re.findall(r"Improvement \d+:\s*(.+?)(?=Improvement \d+:|$)", response, re.DOTALL)
        improvements = [imp.strip() for imp in improvements[:2]]  # Take first 2 improvements
        
        if len(improvements) < 2:
            improvements.extend(["Focus on providing more specific examples", "Practice structuring your responses more clearly"])
        ...
        improvements = ["Try to be more specific in your answer", "Practice explaining concepts more clearly"]
...
def generate_ai_interview_questions(job_role: str, num_questions: int = 8):
    ...
    Returns a list of questions (behavioral + technical + conceptual, NO coding).
    """
    from llm_manager import call_llm
    import streamlit as st
    import re
    ...
    - Questions should feel realistic and professional
    - Focus on experience, problem-solving, and knowledge
    - Each question should be on a separate line starting with "Q:"

    ### Question Types to Include:
    1. Behavioral questions (Tell me about a time...)
    2. Technical knowledge questions (How would you approach...)
    3. Conceptual questions (What is your understanding of...)
    ...
    try:
        ...
        # Extract questions
        questions = re.findall(r"Q:\s*(.+)", response)
        ...
        if len(questions) < num_questions:
            ...
            fallback_questions = [
                f"Tell me about your experience with {job_role.lower()} responsibilities.",
                f"How do you stay updated with the latest trends in {job_role.lower()}?",
                f"Describe a challenging project you worked on in your {job_role.lower()} role.",
                f"How do you prioritize tasks when working on multiple projects?",
                f"What motivates you most about working as a {job_role}?",
                f"How do you handle feedback and criticism in your work?",
                f"Describe your approach to learning new technologies or methodologies.",
                f"How do you ensure quality in your work as a {job_role}?"
            ]
            
            questions.extend(fallback_questions[:num_questions - len(questions)])
        ...
        # Fallback questions
        return [
            f"Tell me about your experience and interest in the {job_role} role.",
            f"How do you approach problem-solving in {job_role.lower()} work?",
            f"Describe a challenging situation you faced and how you handled it.",
            f"How do you stay current with industry trends and best practices?",
            f"What do you consider your greatest strength as a {job_role}?",
            f"How do you handle working under pressure or tight deadlines?",
            f"Describe your experience working in a team environment.",
            f"What motivates you most about this field and role?"
        ]
...
with tab4:
    ...
    BADGE_CONFIG = {
        ...
        "interview": {
            "needs_practice": {"min_score": 1.0, "max_score": 4.0, "emoji": "💪", "title": "Keep Practicing"},
            "good": {"min_score": 4.1, "max_score": 6.0, "emoji": "👍", "title": "Good Performer"},
            "excellent": {"min_score": 6.1, "max_score": 8.0, "emoji": "🌟", "title": "Star Performer"},
            "interview_ready": {"min_score": 8.1, "max_score": 10.0, "emoji": "🏆", "title": "Interview Ready"}
        }
    }
    ...
    # Section 4: AI Interview Coach 🤖 - COMPLETELY REWRITTEN
        ...
        if 'ai_current_question' not in st.session_state:
            st.session_state.ai_current_question = 0
        ...
        if 'ai_interview_improvements' not in st.session_state:
            st.session_state.ai_interview_improvements = []
        ...
        if 'ai_timer_start' not in st.session_state:
            st.session_state.ai_timer_start = None
        if 'ai_timer_expired' not in st.session_state:
            st.session_state.ai_timer_expired = False
        if 'ai_selected_role' not in st.session_state:
            st.session_state.ai_selected_role = None

        # Job role input and setup
        if not st.session_state.ai_interview_started:
            ...
            if job_role and st.button("🚀 Start AI Interview Practice", key="start_ai_interview"):
                with st.spinner("🤖 Generating personalized interview questions..."):
                    # Generate AI questions
                    questions = generate_ai_interview_questions(job_role, num_questions)
                    
                    if questions:
                        st.session_state.ai_interview_questions = questions
                        st.session_state.ai_selected_role = job_role
                        st.session_state.ai_current_question = 0
                        st.session_state.ai_interview_answers = []
                        st.session_state.ai_interview_scores = []
                        st.session_state.ai_interview_feedback = []
                        ...
                        st.session_state.ai_interview_completed = False
                        st.session_state.ai_answer_submitted = False
                        st.session_state.ai_timer_start = time.time()
                        st.session_state.ai_timer_expired = False
                        st.success(f"✅ Generated {len(questions)} questions for {job_role}!")
                        st.rerun()
                    else:
        ...
        # Interview in progress
            ...
            if st.session_state.ai_current_question < len(st.session_state.ai_interview_questions):
                current_q = st.session_state.ai_interview_questions[st.session_state.ai_current_question]
                
                # Question display
                st.markdown(f"""
                <div class="quiz-card">
                    <h3 style="color: #00c3ff;">Question {st.session_state.ai_current_question + 1} of {len(st.session_state.ai_interview_questions)}</h3>
                    <h4 style="color: #ffffff; margin: 15px 0;">Role: {st.session_state.ai_selected_role}</h4>
                    <p style="font-size: 18px; color: #ffffff; margin: 15px 0;">{current_q}</p>
                </div>
                """, unsafe_allow_html=True)
                
                # Timer logic
                if not st.session_state.ai_answer_submitted and not st.session_state.ai_timer_expired:
                    if st.session_state.ai_timer_start:
                        elapsed = time.time() - st.session_state.ai_timer_start
                        remaining = max(0, 90 - elapsed)  # 90 seconds timer
                        ...
                        if remaining > 0:
                            ...
                            if remaining > 30:
                                timer_color = "#00c3ff"  # Blue
                            elif remaining > 10:
                                timer_color = "#ff9800"  # Orange
                            else:
                                timer_color = "#ff4500"  # Red
                            ...
                            st.session_state.ai_timer_expired = True
                            st.rerun()
                
                # Answer input (disabled if timer expired)
                answer_disabled = st.session_state.ai_timer_expired or st.session_state.ai_answer_submitted
                
                if st.session_state.ai_timer_expired and not st.session_state.ai_answer_submitted:
                    st.warning("⏰ Time's up! Your answer input has been locked.")
                
                answer = st.text_area(
                    "Your answer:",
                    placeholder="Type your detailed answer here..." if not answer_disabled else "Time expired or answer already submitted",
                    height=150,
                    disabled=answer_disabled,
                    key=f"ai_answer_{st.session_state.ai_current_question}"
                )
                ...
                if not st.session_state.ai_answer_submitted:
                    ...
                    with col1:
                        if st.button("Submit Answer & Get AI Feedback", disabled=answer_disabled):
                            if answer.strip() or st.session_state.ai_timer_expired:
                                with st.spinner("🤖 AI is evaluating your answer..."):
                                    # Evaluate answer with AI
                                    score, feedback, improvements = evaluate_interview_answer(answer if answer.strip() else "No answer provided due to time limit.")
                                    ...
                                    st.session_state.ai_interview_improvements.append(improvements)
                    ...
                    with col2:
                        if st.button("Skip Question", key=f"skip_{st.session_state.ai_current_question}"):
                            ...
                            st.session_state.ai_interview_improvements.append(["Try to answer all questions", "Practice time management"])
                            st.session_state.ai_answer_submitted = True
                            st.rerun()
                
                # Show feedback after submission
                else:
                    score = st.session_state.ai_interview_scores[st.session_state.ai_current_question]
                    feedback = st.session_state.ai_interview_feedback[st.session_state.ai_current_question]
                    improvements = st.session_state.ai_interview_improvements[st.session_state.ai_current_question]
                    
                    st.markdown(f"""
                    <div class="feedback-container">
                        <h4 style="color: #00c3ff;">🤖 AI Feedback:</h4>
                        <p style="color: #ffffff; font-size: 18px; margin: 10px 0;">📊 Score: {score}/10</p>
                        <p style="color: #ffffff; margin: 10px 0;"><strong>Feedback:</strong> {feedback}</p>
                        <div style="margin: 15px 0;">
                            <p style="color: #00c3ff; font-weight: 600;">💡 Suggestions for Improvement:</p>
                            <ul style="color: #ffffff; margin-left: 20px;">
                                <li>{improvements[0] if len(improvements) > 0 else 'Keep practicing'}</li>
                                <li>{improvements[1] if len(improvements) > 1 else 'Focus on clarity'}</li>
                            </ul>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    if st.session_state.ai_current_question < len(st.session_state.ai_interview_questions) - 1:
                        if st.button("Next Question ➡️", key="ai_next_question"):
                            st.session_state.ai_current_question += 1
                            st.session_state.ai_answer_submitted = False
                            st.session_state.ai_timer_start = time.time()
                            st.session_state.ai_timer_expired = False
                            st.rerun()
                    else:
                        if st.button("Complete Interview 🏁", key="ai_complete_interview"):
                            st.session_state.ai_interview_completed = True
                            st.rerun()
                
                # Progress bar
                progress = (st.session_state.ai_current_question + 1) / len(st.session_state.ai_interview_questions)
                st.progress(progress)

        # Interview completed - show results
        elif st.session_state.ai_interview_completed:
            # Calculate average score
            valid_scores = [s for s in st.session_state.ai_interview_scores if s > 0]
            ...
            <div class="badge-container">
                ...
                <p style="color: #666;">Role: {st.session_state.ai_selected_role}</p>
                <p style="color: #666;">Questions Completed: {len(st.session_state.ai_interview_scores)}</p>
            </div>
            """, unsafe_allow_html=True)
            
            # Detailed results
            st.subheader("📊 Detailed AI Feedback:")
            for i, (question, answer, score, feedback, improvements) in enumerate(zip(
                st.session_state.ai_interview_questions,
                st.session_state.ai_interview_answers,
                st.session_state.ai_interview_scores,
                st.session_state.ai_interview_feedback,
                ...
                with st.expander(f"Question {i+1}: Score {score}/10"):
                    ...
                    st.write(f"**Your Answer:** {answer[:200]}{'...' if len(answer) > 200 else ''}")
                    st.write(f"**AI Score:** {score}/10")
                    st.write(f"**AI Feedback:** {feedback}")
                    st.write("**Improvement Suggestions:**")
                    for j, improvement in enumerate(improvements, 1):
                        st.write(f"{j}. {improvement}")
            
            # Restart button
            col1, col2 = st.columns(2)
            with col1:
                if st.button("🔄 Practice Again", key="ai_restart"):
            ...
            with col2:
                if st.button("📚 View Recommended Courses", key="ai_view_courses"):
                    st.session_state.page_selection = "Courses by Role"
                    st.rerun()
