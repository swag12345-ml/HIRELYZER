...
with tab2:
    ...
    # ✅ Dynamic sections (Outside the form)
    ...
    for idx, edu in enumerate(st.session_state.education_entries):
        with st.expander(f"Education #{idx+1}", expanded=True):
            edu["degree"] = st.text_input(f"Degree {idx+1}", value=edu["degree"], key=f"degree_{idx}")
            edu["institution"] = st.text_input(f"Institution {idx+1}", value=edu["institution"], key=f"institution_{idx}")
            edu["year"] = st.text_input(f"Year {idx+1}", value=edu["year"], key=f"edu_year_{idx}")
            edu["details"] = st.text_area(f"Details {idx+1}", value=edu["details"], key=f"edu_details_{idx}")
    ...
    for idx, proj in enumerate(st.session_state.project_entries):
        with st.expander(f"Project #{idx+1}", expanded=True):
            proj["title"] = st.text_input(f"Project Title {idx+1}", value=proj["title"], key=f"proj_title_{idx}")
            proj["tech"] = st.text_input(f"Tech Stack {idx+1}", value=proj["tech"], key=f"proj_tech_{idx}")
            proj["duration"] = st.text_input(f"Duration {idx+1}", value=proj["duration"], key=f"proj_duration_{idx}")
            proj["description"] = st.text_area(f"Description {idx+1}", value=proj["description"], key=f"proj_desc_{idx}")
    ...
    for idx, cert in enumerate(st.session_state.certificate_links):
        with st.expander(f"Certificate #{idx+1}", expanded=True):
            cert["name"] = st.text_input(f"Certificate Name {idx+1}", value=cert["name"], key=f"cert_name_{idx}")
            cert["link"] = st.text_input(f"Certificate Link {idx+1}", value=cert["link"], key=f"cert_link_{idx}")
            cert["duration"] = st.text_input(f"Duration {idx+1}", value=cert["duration"], key=f"cert_duration_{idx}")
            cert["description"] = st.text_area(f"Description {idx+1}", value=cert["description"], key=f"cert_description_{idx}")
